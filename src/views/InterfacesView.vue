<template>
  <div class="page-stack">
    <n-card title="外部上传接口说明" size="small" class="content-card">
      <div class="api-box">
    ...
    <n-card title="上传字段映射配置" size="small" class="content-card">
      ...
      <div class="section-actions">
        <n-button type="primary" :loading="saveIngestLoading" @click="saveIngestConfig">
          保存映射配置
        </n-button>
      </div>
    </n-card>

    <n-card title="请求示例" size="small" class="content-card">
      <n-space vertical>
    ...
    <n-card title="接口总览" size="small" class="content-card">
    ...
    <n-card title="开放取件 API（Token 鉴权）" size="small" class="content-card">
      <n-space vertical>
        <p class="hint">支持 Header：{{ mailApiTokenHeader }} 或 Authorization: Bearer token。</p>
        <p class="hint">管理后台默认使用 <code>mode=auto</code>，会优先尝试 Graph，失败后回退 Outlook/IMAP 兼容读取。</p>
    ...
    <n-card title="管理端 API（登录会话）" size="small" class="content-card">
      <n-space vertical>
