...
async function saveProxyConfig(): Promise<void> {
  ...
  try {
    ...
    const response = await api.updateSub2ApiLongLinkConfig(buildProxyConfigPayload());
    applyProxyConfig(response.item);
    proxyDraft.value = '';
...
async function testAllProxyItems(): Promise<void> {
  ...
  if (proxyItems.value.length === 0) {
    syncProxyPoolFromItems();
    await checkNetworkProxyPool(proxyPool.value);
    return;
  }
...
async function checkNetworkProxyPool(pool: string): Promise<void> {
  proxyChecking.value = true;
  proxyStatusTone.value = 'default';
  ...
  try {
    ...
    proxyStatus.value = `网络代理池未生效：${getErrorMessage(error)}`;
    proxyStatusTone.value = 'error';
    message.error(getErrorMessage(error));
  } finally {
    proxyChecking.value = false;
